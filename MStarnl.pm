...
# URLs
use vars qw($VERSION $MSTARNL_NEXT_URL $MSTARNL_LOOK_UP $MSTARNL_MAIN_URL);
...
$MSTARNL_MAIN_URL   =   "http://www.morningstar.nl";
$MSTARNL_LOOK_UP    =   "http://www.morningstar.nl/nl/funds/SecuritySearchResults.aspx?search=";
$MSTARNL_NEXT_URL	=	"http://www.morningstar.nl/nl/funds/snapshot/snapshot.aspx?id=";

# FIXME -

sub methods { return (mstarnl => \&mstarnl_fund,); }

{
    my @labels = qw/name currency last date time price nav source iso_date method net p_change success errormsg/;

    sub labels { return (mstarnl => \@labels); }
}
...
#
        ...
        my $webdoc  = get($MSTARNL_LOOK_UP.$code);
        if (!$webdoc)
        {
	    ...
	    $fundquote {$code, "source"} = $MSTARNL_MAIN_URL;
...
# Now need to look-up next page using $next_url
        ...
        $webdoc  = get($MSTARNL_MAIN_URL.$nexturl);
        if (!$webdoc)
        {
	        # serious error, report it and give up
		    $fundquote {$code,"success"} = 0;
...
# Find date, currency and price all in one table row
		...
		if ($webdoc =~
		m[<td class="line heading">Koers<span class="heading"><br />([0-9]{2}-[0-9]{2}-[0-9]{4})</span>.*([A-Z]{3}).([0-9\,]+).*>([0-9,\-]+)] )
        {
            $date = $1;
            $currency = $2;
            $price = $3;
...
# Set a dummy time as gnucash insists on having a valid format
		...
		$fundquote {$code, "method"} = "mstarnl";   # set method
...
Finance::Quote::mstarnl - Obtain UK Unit Trust quotes from morningstar.nl.

=head1 SYNOPSIS

    $q = Finance::Quote->new;

    %info = Finance::Quote->fetch("mstarnl","<isin> ...");  # Only query morningstar.nl using ISINs

=head1 DESCRIPTION

This module fetches information from the MorningStar Funds service,
http://morningstar.com/nl/. There are many UK Unit Trusts and OEICs quoted,
...
also possible to load it explicity by placing "mstarnl" in the argument
list to Finance::Quote->new().

Information obtained by this module may be covered by funds.ft.com
terms and conditions See http://morningstar.nl for details.

=head2 Stocks And Indices

This module provides "mstarnl".
=head1 LABELS RETURNED

The following labels may be returned by Finance::Quote::mstarnl :
...
Morning Star websites, http://morningstar.nl
...
Copyright (C) 2017 by Thomas Jollans
